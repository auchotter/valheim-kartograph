...
const objectSelect = `
  SELECT
    ...
    l.reference_zoom, l.rotation_degrees
  FROM map_objects mo
...
export class MapRepository {
  ...
  private writeSubtype(object: MapObject): void {
    ...
    this.database
      .prepare(
        `INSERT INTO labels (
          object_id, x, y, text, font_size, reference_zoom, rotation_degrees
        ) VALUES (?, ?, ?, ?, ?, ?, ?)`,
      )
      .run(
        object.id,
...
function toMapObject(row: ObjectRow): MapObject {
  ...
  return {
    ...
    referenceZoom: required(row.reference_zoom),
